...
class ECBD:
    ...
    def compute_k(self):
        k = scalar_mult(self.nr_of_participants * self.private_key, self.z_list[(self.id - 1) % self.nr_of_participants])
        index_list = [id % self.nr_of_participants for id in range(self.id, self.id + self.nr_of_participants)]

        for i in index_list:
            k = point_add(k, scalar_mult(self.nr_of_participants - (i+1), self.x_list[i]))
